    ...
    class UserViewModel : ViewModel() {
        ...
        fun listenToUser(userId : String,context: Context){
            userListener?.let { users.child(userId).removeEventListener(it)}
            ...
            userListener = object : ValueEventListener{
                ...
                override fun onCancelled(error: DatabaseError) {
                    Log.e("UserViewModel", "Error: ${error.message}")
                }
            }

            users.child(userId).addValueEventListener(userListener as ValueEventListener)
